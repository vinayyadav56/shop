...
/**
 * PlantAtHome brand header — ported from the prototype Nav.
 * Transparent over the hero (home/vertical pages) → solid cream on scroll;
 * solid + sticky on every other page. Wired to the REAL cart drawer, login
 * modal and search so nothing functional is lost.
 */
const Header = ({ layout }: { layout?: string }) => {
  ...
  // shared with the bottom mobile-nav search button
  ...
  const openMobileMenu = () =>
    setDrawer({ display: true, view: 'MAIN_MENU_VIEW' });
  const onProfile = () => {
