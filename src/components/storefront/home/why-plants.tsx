...
export function WhyPlants() {
  ...
  return (
    <section className="bg-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 sm:px-8 lg:grid-cols-[0.85fr_1.4fr] lg:items-center lg:gap-12 lg:py-20">
        {/* left — editorial copy */}
        <div>
          <span className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.22em] text-gold">
            Why Plants? <Icon.leaf className="h-3.5 w-3.5 text-gold" />
          </span>
          <h2 className="font-cormorant mt-3 text-[2.1rem] font-bold not-italic leading-[1.05] text-forest-900 sm:text-[2.8rem]">
          ...
          <Link
            href="/plants/search"
            className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-md bg-forest-800 px-6 py-3 text-[11.5px] font-semibold uppercase tracking-[0.12em] text-white transition hover:bg-forest-700 sm:w-auto"
          >
        ...
        <div className="relative min-w-0">
          ...
          >
            {CARDS.map((c) => (
              <div
                key={c.title}
                className="w-[72%] shrink-0 snap-start overflow-hidden rounded-xl border border-kraft-200/70 bg-white shadow-[0_18px_40px_-24px_rgba(34,48,26,0.35)] sm:w-[42%] lg:w-[24%]"
              >
          ...
          <button
            ...
            onClick={scrollRight}
            className="absolute right-0 top-1/2 hidden h-9 w-9 -translate-y-1/2 translate-x-1/2 place-items-center rounded-full border border-kraft-300 bg-white shadow-sm transition hover:bg-cream-50 lg:grid"
          >
