...
export function Hero({
  ...
  return (
    <section ref={ref} className="relative h-[100svh] min-h-[640px] w-full overflow-hidden">
      ...
      {/* subtle scrim that blends the dark glass navbar into the hero */}
      <div className="pointer-events-none absolute inset-x-0 top-0 z-[1] h-28 bg-gradient-to-b from-deep/45 to-transparent" />
      ...
      >
        ...
        >
          <Link href={primaryTo} scroll={false}>
            <Magnetic className="inline-flex items-center gap-2 rounded-full bg-leaf px-7 py-4 text-sm font-bold text-white shadow-[0_14px_40px_rgba(67,160,71,0.45)]">
              {primary} <Icon.arrow className="h-4 w-4" />
            </Magnetic>
          </Link>
          <Magnetic
            onClick={() => setTour(true)}
            className="inline-flex items-center gap-2 rounded-full border border-white/35 bg-white/10 px-6 py-4 text-sm font-semibold text-white backdrop-blur-md"
          >
        ...
        >
          {['Same-day delivery', '30-day guarantee', 'Hand-picked'].map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs font-medium text-white/90 backdrop-blur-md"
            >
              {t}
            </span>
          ))}
