...
const STEPS = [
  { img: '/hero-villa-exterior.jpg', title: 'Tell us about your space', text: 'Share your balcony, terrace or backyard and what you dream of. Takes 30 seconds.' },
  { img: '/plants-1.jpg', title: 'We design your package', text: 'Our botanists build a plan tailored to your light, space and budget — plants, soil, tools, visits.' },
  { img: '/farm-2.jpg', title: 'We plant & maintain', text: 'Experts set it up and visit on schedule to keep everything thriving. You just enjoy it.' },
];
...
function LeadForm() {
  ...
  if (done) {
    return (
      <div className="rounded-2xl bg-white p-8 text-center shadow-xl">
        ...
        <h3 className="font-hanken text-xl font-bold text-forest-900">Request received!</h3>
        <p className="mt-2 text-stone-600">Our garden experts will call you within one business day to design your free plan.</p>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl bg-white p-6 shadow-xl sm:p-8">
      <h3 className="font-hanken text-xl font-bold text-forest-900">Get your free garden plan</h3>
      <p className="mt-1 text-sm text-stone-500">No cost, no obligation. We’ll call you within a day.</p>
      <div className="mt-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <select {...register('garden_type')} className="rounded-lg border border-gray-300 px-3 py-3 text-stone-600 focus:border-green-500 focus:outline-0">
            <option value="">Garden type</option>
            {GARDEN_TYPES.map((t) => <option key={t} value={t.toLowerCase()}>{t}</option>)}
          </select>
          <select {...register('budget_range')} className="rounded-lg border border-gray-300 px-3 py-3 text-stone-600 focus:border-green-500 focus:outline-0">
  ...
  return (
    <div className="bg-white">
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-5 py-16 sm:px-8 lg:grid-cols-2 lg:py-24">
          <div className="text-white">
            ...
            <h1 className="mt-5 font-pahserif text-4xl font-bold leading-tight sm:text-5xl">
      ...
      <section className="border-b border-gray-100 bg-green-50/60">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-4 px-5 py-6 text-center sm:px-8 md:grid-cols-4">
          {[['2,000+', 'Gardens delivered'], ['4.9★', 'Average rating'], ['100%', 'Plant-health guarantee'], ['Same-day', 'Metro delivery']].map(([a, b]) => (
            <div key={b}><div className="font-pahserif text-2xl font-bold text-forest-700">{a}</div><div className="text-sm text-stone-600">{b}</div></div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <h2 className="text-center font-pahserif text-3xl font-bold text-forest-900">How it works</h2>
        <p className="mx-auto mt-2 max-w-2xl text-center text-stone-500">From bare space to thriving garden in three simple steps.</p>
        <div className="mt-10 grid gap-8 md:grid-cols-3">
          {STEPS.map((s, i) => (
            <div key={s.title} className="overflow-hidden rounded-2xl border border-gray-100 shadow-sm">
              ...
              <div className="p-6">
                <div className="mb-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-forest-700 text-sm font-bold text-white">{i + 1}</div>
                <h3 className="font-hanken text-lg font-bold text-forest-900">{s.title}</h3>
      ...
      <section className="bg-gray-50 py-16">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <h2 className="text-center font-pahserif text-3xl font-bold text-forest-900">Real transformations</h2>
          <div className="mt-10 grid gap-6 sm:grid-cols-2">
            <figure className="relative overflow-hidden rounded-2xl">
              <img src="/plants-3.jpg" alt="Before" className="h-72 w-full object-cover" />
            ...
            <figure className="relative overflow-hidden rounded-2xl">
              <img src="/farm-1.jpg" alt="After" className="h-72 w-full object-cover" />
              <figcaption className="absolute left-4 top-4 rounded-full bg-forest-700 px-3 py-1 text-xs font-semibold text-white">After</figcaption>
            </figure>
          </div>
        </div>
      </section>
      ...
      {/* WHAT'S INCLUDED */}
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <h2 className="text-center font-pahserif text-3xl font-bold text-forest-900">Everything’s included</h2>
        <p className="mx-auto mt-2 max-w-2xl text-center text-stone-500">One package, zero hassle. We bring the green thumb.</p>
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {INCLUDED.map((f) => (
            <div key={f.title} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="text-3xl">{f.icon}</div>
              <h3 className="mt-3 font-hanken font-bold text-forest-900">{f.title}</h3>
      ...
      <section className="bg-green-950 py-16 text-white">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <h2 className="text-center font-pahserif text-3xl font-bold">Popular garden packages</h2>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-3">
            {(templates.length ? templates : []).map((t, i) => (
              <div key={t.id} className={`rounded-2xl bg-white/5 p-7 ring-1 ${i === 1 ? 'ring-2 ring-green-400' : 'ring-white/10'}`}>
                {i === 1 && <div className="mb-3 inline-block rounded-full bg-[#8FD56F] px-3 py-1 text-xs font-bold text-forest-900">MOST POPULAR</div>}
                <h3 className="font-hanken text-xl font-bold">{t.name}</h3>
                <p className="mt-1 text-sm text-green-100/70">{t.tagline}</p>
                <div className="mt-4 text-3xl font-bold">From {fmt(t.suggested_price)}</div>
      ...
      {/* TESTIMONIALS */}
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <h2 className="text-center font-pahserif text-3xl font-bold text-forest-900">Loved by plant parents</h2>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {[
            ...
            <div key={n} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-stone-600">“{q}”</p>
              <div className="mt-4 text-sm font-semibold text-forest-900">{n} <span className="font-normal text-gray-400">· {c}</span></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gray-50 py-16">
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          <h2 className="text-center font-pahserif text-3xl font-bold text-forest-900">Questions, answered</h2>
          <div className="mt-8 space-y-3">
            {FAQS.map((f) => (
              <details key={f.q} className="group rounded-xl border border-gray-200 bg-white p-5">
                <summary className="cursor-pointer list-none font-semibold text-forest-900 marker:hidden">{f.q}</summary>
      ...
      {/* FINAL CTA */}
      <section className="bg-forest-700 py-14 text-center text-white">
        <div className="mx-auto max-w-3xl px-5">
          <h2 className="font-pahserif text-3xl font-bold">Ready for a garden you’ll actually keep alive?</h2>
          <p className="mt-2 text-green-50/90">Get a free, no-obligation plan tailored to your space today.</p>
          <a href="#quote" className="pa-btn pa-btn-outline pa-btn-lg mt-6 border-white bg-white text-forest-700 shadow-lg hover:bg-forest-800 hover:text-white">Get my free garden plan →</a>
        </div>
      </section>
    </div>
  );
